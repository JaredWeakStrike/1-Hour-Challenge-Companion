...
function _OnInit()
GameVersion = 0
print('Pride Lands 2 Simba Oasis skip')
end
...
if (ReadByte(Save + 0x370A) & (0x1 << 2)) > 0  and (ReadByte(Save + 0x3707) & (0x1 << 6)) == 0  then -- if hyneas get bonus is done and groundshakers is not
	WriteByte(Save+0x0F46+4,0) --despawn the oasis cutscene
	WriteByte(Save+0x0F10+4,6) --spawn groundshaker
end
